    ...
    *,
    ...
    for race in races:
        affected = [
            ...
            if r.get("finishing_position") is None and not r.get("non_runner")
        ]
        if affected:
    ...
    if dry_run:
        logger.info(
            f"Found {total_runners} non-runner(s) across {len(race_ids)} race(s)."
        )
    ...
    for race_id in race_ids:
        ...
        for i, runner in enumerate(race.get("runners", [])):
            if runner.get("finishing_position") is None and not runner.get(
                "non_runner"
            ):
